...
''') {
  node(POD_LABEL) {
    ...
    //writeFile file: 'Dockerfile', text: 'FROM scratch'
        stage('docker build') {
               container('docker'){
                  sh 'docker version && DOCKER_BUILDKIT=1 docker build --progress plain -t qa-docker-nexus.mtnsat.io/dockerrepo/testing:latest .'                   
               }
        }
        stage('docker login') {
                container('docker') {
        ...
        stage('docker push'){
               container('docker'){
                   sh 'docker push qa-docker-nexus.mtnsat.io/dockerrepo/testing:latest'
               }
        }
  }
}
